...
ctrl_thread__next_dmn_event(Arena *arena, DMN_CtrlCtx *ctrl_ctx, CTRL_Msg *msg, DMN_RunCtrls *run_ctrls, CTRL_Spoof *spoof)
{
  ...
  ProfScope("loop -> try to get event, run, repeat") for(B32 got_event = 0; got_event == 0;)
  {
    ...
    if(got_event == 0) ProfScope("no event -> dmn_ctrl_run for a new one")
    {
      ...
      {
        LogInfoNamedBlock("dmn_ctrl_run")
        {
          ...
          LogInfoNamedBlock("run_entities") for(U64 idx = 0; idx < run_ctrls->run_entity_count; idx += 1)
          {
            log_infof("[0x%I64x]\n", run_ctrls->run_entities[idx]);
          }
          log_infof("trap_count:                 %I64u\n", run_ctrls->traps.trap_count);
          LogInfoNamedBlock("traps") for(DMN_TrapChunkNode *n = run_ctrls->traps.first; n != 0; n = n->next)
          {
...
ctrl_thread__run(DMN_CtrlCtx *ctrl_ctx, CTRL_Msg *msg)
{
  ...
  log_info(str8_lit("ctrl_thread__run:\n{\n"));
  ...
  if(stop_event == 0)
  {
    ...
    for(;;)
    {
      ...
      //
      log_info(str8_lit("get_next_event:\n{\n"));
      ...
      switch(event->kind)
      {
        ...
        {
          hard_stop = 1;
          log_info(str8_lit("step_rule: unexpected -> hard_stop\n"));
        }break;
        case DMN_EventKind_Exception:
        case DMN_EventKind_Breakpoint:
        {
          use_stepping_logic = 1;
          log_info(str8_lit("step_rule: exception/breakpoint -> stepping_logic\n"));
        }break;
        case DMN_EventKind_CreateProcess:
        {
          ...
          log_info(str8_lit("step_rule: create_process -> resolve traps\n"));
          log_info(str8_lit("new_traps:\n{\n"));
        ...
        case DMN_EventKind_LoadModule:
        {
          ...
          log_info(str8_lit("step_rule: load_module -> resolve traps\n"));
          log_info(str8_lit("new_traps:\n{\n"));
          for(DMN_TrapChunkNode *n = new_traps.first; n != 0; n = n->next)
          {
            for(U64 idx = 0; idx < n->count; idx += 1)
      ...
      if(hit_spoof)
      {
        log_info(str8_lit("exit_spoof_mode\n"));
        ...
        if(conditions.node_count != 0) ProfScope("evaluate hit stop conditions")
        {
          ...
          for(String8Node *condition_n = conditions.first; condition_n != 0; condition_n = condition_n->next)
          {
            ...
            if(eval.code == E_InterpretationCode_Good && eval.value.u64 == 0)
            {
              ...
              log_info(str8_lit("conditional_breakpoint_hit: 'condition eval'd to 0, and so filtered'\n"));
            }
            else
            {
              hit_user_bp = 1;
              hit_conditional_bp_but_filtered = 0;
              log_info(str8_lit("conditional_breakpoint_hit: 'conditional eval'd to nonzero, hit'\n"));
              break;
            }
          }
          di_scope_close(di_scope);
      ...
      if(hit_conditional_bp_but_filtered) LogInfoNamedBlock("conditional_bp_hit_single_step")
      {
      ...
      {
        if(hit_trap_flags & CTRL_TrapFlag_SingleStepAfterHit) LogInfoNamedBlock("trap_net__single_step_after_hit")
        {
      ...
      if(!hard_stop && use_trap_net_logic)
      {
        if(hit_trap_flags & CTRL_TrapFlag_BeginSpoofMode) LogInfoNamedBlock("trap_net__begin_spoof_mode")
        {
      ...
      if(!hard_stop && use_trap_net_logic)
      {
        ...
        {
          if(stack_pointer_matches) LogInfoNamedBlock("trap_net__save_sp")
          {
      ...
      if(!hard_stop && use_trap_net_logic)
      {
        if(hit_trap_flags & CTRL_TrapFlag_EndStepping) LogInfoNamedBlock("trap_net__end_step")
        {
      ...
      if(step_past_trap_net) LogInfoNamedBlock("trap_net__single_step_past_trap_net")
      {
        DMN_Handle thread = event->thread;
        U64 thread_pre_rip = dmn_rip_from_thread(thread);
        U64 thread_post_rip = thread_pre_rip;
